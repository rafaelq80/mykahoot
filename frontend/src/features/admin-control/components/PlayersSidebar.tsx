...
export function PlayersSidebar() {
  ...
  return (
    <aside className="flex w-full shrink-0 flex-col gap-3 rounded-2xl border-2 border-white/20 bg-white shadow-md lg:w-64 xl:w-72">
      <div className="flex items-center justify-between border-b border-surface-container px-4 pb-3 pt-5">
      ...
      <div className="flex max-h-80 flex-col gap-1.5 overflow-y-auto px-3 pb-4 lg:max-h-none lg:flex-1">
        {players.length === 0 && (
          <p className="rounded-xl bg-surface-container px-3 py-3 text-center text-xs font-medium text-gray-400">
        ...
        {players.map((p) => (
          ...
          >
            ...
            <span className="truncate text-sm font-bold text-gray-800">{p.nickname}</span>
          </div>
        ))}
      </div>
    </aside>
